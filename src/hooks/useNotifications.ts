...
export function useNotifications(events: FestivalEvent[], getMeta: (id: string) => UserMeta, now: Date) {
  ...
  React.useEffect(() => {
    ...
    selected.forEach(event => {
      ...
      if (meta.notify30 && mins === 30) notify('30', `${event.artist} dans 30 min`, `${event.stage} · ${event.start}`);
      if (meta.notify10 && mins === 10) notify('10', `${event.artist} dans 10 min`, `${event.stage} · prépare-toi`);
      if (meta.notifyStart && mins === 0) notify('start', `${event.artist} commence`, `${event.stage} · maintenant`);
    });
